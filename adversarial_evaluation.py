  ...
  class EvaluationResult(object):
      ...
      def __init__(self, attack_params, to_eval=None,
          ...
          shorthand_evals = {'top1': self.top1_accuracy,
                             ...
                             'avg_successful_linf': self.avg_successful_linf,
                             'stash_perturbations': self.stash_perturbations,
                             'avg_loss_value': self.avg_loss_value,
                              'stash_adversarials': self.stash_adversarials,
                             'stash_labels': self.stash_labels
                            }
          if to_eval is None:
              to_eval = {'top10': 'top10'}
      ...
      def top10_accuracy(self, eval_label, attack_out):
      ...
      def avg_successful_ssim(self, eval_label, attack_out):
          ...
          avg_minus_ssim = runsum / float(count)
      ...
      def stash_adversarials(self, eval_label, attack_out):
          ####This will store the perturbations.
           ####  (TODO: make these tensors and store on CPU)
          ...
          result = self.results[eval_label]
          adversarials = attack_out[0]
          result.append(adversarials)
      def stash_labels(self, eval_label, attack_out):
          ####This will store the perturbations.
           ####  (TODO: make these tensors and store on CPU)
  ...
  class AdversarialEvaluation(object):
      ...
      def evaluate_ensemble(self, data_loader, attack_ensemble,
                            skip_ground=False, verbose=True,output_filename,
                            num_minibatches=None,save_xform=img_utils.nhwc255_xform):
          ...
          for i, data in enumerate(data_loader, 0):
              ...
              for k, result in attack_ensemble.items():
                  ...
                  attack_out = result.eval(inputs, labels)
                  adv_data = attack_out.cpu().numpy()
                  minibatch_attacks[k].append(save_xform(adv_data))
                  if i = num_minibatches-1:

                    output_file = os.path.join(output_filename,k + '.npy')
                    minibatch_attacks = utils.checkpoint_incremental_array(
                                                   output_file, minibatch_attacks[k],
                                                   return_concat=True)
